...
/// macOS's Screen Recording permission, which Stackshot needs to freeze the screen itself.
@MainActor
enum ScreenCapturePermission {
    static let settingsURL = URL(string: "x-apple.systempreferences:com.apple.preference.security?Privacy_ScreenCapture")!

    /// True when Stackshot may capture the screen. The first time, asks macOS to show its own prompt;
